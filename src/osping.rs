    ...
    match cmd.arg(host).output() {
        Ok(output) => {
            if output.status.success() {
                ...
                PingResult::ConnectionFailure(format!(
                    "Host '{}' cannot be reached over a network ICMP Ping",
                    host
                ))
            } else {
                ...
                if stdout.contains("could not find host") {
                    // Windows
                    PingResult::DNSIssue(format!(
                        "Ping returned error indicating no DNS entry for '{}'. OS output received: \
                        '{}'",
                        host, stdout
                    ))
                    ...
                    // Unix
                    PingResult::DNSIssue(format!(
                        "Ping returned error indicating the DNS entry is not a hostname associated \
                        with an IP address. OS output received: '{}'",
                        stderr
                    ))
                } else if cfg!(windows) {
                    // Windows (Window's Ping uses stdout for errors rather than stderr)
                    PingResult::ConnectionFailure(format!(
                        "Ping returned error. OS output received - stdout: '{}' - stderr: '{}'",
                        stdout, stderr
                    ))
                } else {
                    // Unix (Ping on Linux correctly uses stderr)
                    PingResult::ConnectionFailure(format!(
                        "Ping returned error. OS output received: '{}'",
                        stderr
                    ))
                }
            }
        ...
        Err(e) => {
            ...
            if e.kind() == ErrorKind::NotFound {
                PingResult::OSCmndIssue(
                    "Unable to locate 'ping' executable in the local OS environment - ensure this \
                    executable is on your environment path (check your PATH environment variable)"
                        .to_string(),
                )
            } else if e.kind() == ErrorKind::PermissionDenied {
                PingResult::OSCmndIssue(
                    "Unable to run the 'ping' executable in the local OS \
                    environment due to lack of permissions - ensure the 'ping' command on your OS \
                    is assigned with executable permissions for your OS user running this tool"
                        .to_string(),
                )
            } else {
                PingResult::OSCmndIssue(format!(
                    "Unable to invoke the 'ping' executable on the underlying OS. OS output \
                    received: '{}'",
                    e.to_string()
                ))
            }
