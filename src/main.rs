...
fn main() {
    let args = App::new(APP_TITLE)
        ...
        .about("\nChecks the connectivity from your machine to a remote MongoDB deployment.\n\
               ...
               .\nThe MongoDB deployment may be hosted in Atlas or may be self-managed on-prem / \
               in-cloud.")
        ...
        .after_help("EXAMPLES:\n    \
                    ...
                    \n    \
                    ./mongo-connection-check -u mysyr -p pswd 'mongodb+srv://ctr.a1b2.mongodb\
                    .net/?retryWrites=true'\n\
                    \n    \
                    # Connect to self-managed cluster with seed list of servers in URL\
                    \n    \
                    ./mongo-connection-check 'mongodb://ctr1.acme.com:27017,ctr2.acme.net:27017/\
                    test?tls=true'\n\n")
        .get_matches();
...
async fn run_checks(
    ...
    // STAGE 2:
    let cluster_address =
        stage2_members_check(STAGE2, stages_status, &dns_resolver, url, &cluster_seed_list).await?;
    // STAGE 3:
    let hostname_ipaddr_mappings =
        stage3_dns_ip_check(STAGE3, stages_status, &dns_resolver, &cluster_address).await?;
    // STAGE 4:
...
fn stage1_url_check(
    ...
    let cluster_seed_list = match extract_cluster_seedlist(url) {
        ...
        Err(e) => {
            println!(
                "{}A seed list of target server(s) or SRV service was not found in URL: '{}'",
                ERR_MSG_PREFIX, url
            );
            stages_status[stage_index].advice.push(
    ...
    println!(
        "{}Deployment seed list specified as: '{}'",
        INF_MSG_PREFIX,
    ...
    if is_srv_url(url) {
        if cluster_seed_list.len() > 1 {
            const MSG: &str = "SRV based server name specified but more than 1 service name is \
                              provided";
            stages_status[stage_index].advice.push(
                "Fix the URL to only have one SRV name defined when using a 'mongodb+srv://' \
                prefix in the URL, or just use a 'mongodb://' prefix instead, and specify one or \
                more server 'hostname:port' combinations, each separated by a comma - for more \
                information on the URL format see: \
                https://docs.mongodb.com/manual/reference/connection-string/"
                    .to_string(),
            );
            ...
            println!(
                "{}The seed list part of the URL is valid as it defines a single SRV service, not \
                multiple",
                INF_MSG_PREFIX
            );
            println!(
                "{}Therefore, cluster SRV service name & port is: '{}'",
                INF_MSG_PREFIX,
                get_displayable_addresses(&cluster_seed_list)
            );
        }
...
async fn stage2_members_check(
    ...
    let cluster_addresses = if is_srv_url(url) {
        ...
        println!(
            "{}The seed list in the URL is based on raw host addresses rather than a service name, \
            so no need to perform a DNS SRV lookup",
            INF_MSG_PREFIX
        );
    ...
    println!(
        "{}Deployment seed list now {}: '{}'",
        INF_MSG_PREFIX,
        phrasing,
        get_displayable_addresses(&cluster_addresses)
    );
    print_address_list_members(
        "Deployment individual raw server address identified",
        &cluster_addresses,
    );
    stages_status[stage_index].state = StageState::Passed;
...
async fn stage3_dns_ip_check(
    ...
    const MSG: &str = "Unable to find any IP address mapping in DNS for any of the members of the \
                      deployment";
    const ADVC: &str =
        "From this machine launch a terminal and use the nslookup tool to query DNS for the IP \
        address of the server hostname  (if nothing is returned, then you have a DNS problem):  \
        'nslookup";
        ...
        Ok(hostname_ipaddress_mappings) => {
            ...
            for hostname_ipaddress_mapping in hostname_ipaddress_mappings.iter() {
                match hostname_ipaddress_mapping.ipaddress {
                    ...
                    None => {
                        println!(
                            "{}No IPv4 IP address found in DNS for hostname: '{}'",
                            WRN_MSG_PREFIX, hostname_ipaddress_mapping.hostname
                        );
                        stages_status[stage_index]
                            .advice
                            .push(format!("{} {}'", ADVC, hostname_ipaddress_mapping.hostname));
                    }
                };
            }
...
//
async fn stage4_ip_socket_check(
    stage_index: usize, stages_status: &mut [StageStatus],
    ...
    for hostnm_ipaddr_map in hostname_ipaddr_maps {
        ...
        let ipaddress = match hostnm_ipaddr_map.ipaddress {
            ...
            None => {
                println!(
                    "{}Skipping attempt to open a TCP socket connection to '{}:{}' because its IP \
                    address was not previously resolved in DNS (see warning in previous stage",
                    INF_MSG_PREFIX, &hostnm_ipaddr_map.hostname, port
                );
                continue;
            }
        };

        let fut = task::spawn(try_open_client_tcp_connection(
            hostnm_ipaddr_map.hostname.clone(),
            *port,
            ipaddress,
        ));
        futures.push(fut);
    ...
    for fut in joined_futures {
        ...
        connect_success_count += match ip_check_result.result {
            Ok(_) => {
                println!(
                    "{}TCP socket connection successfully opened to server '{}:{}' (IP address: \
                    '{}')",
                    INF_MSG_PREFIX,
                    ip_check_result.hostname,
                    ip_check_result.port,
                    ip_check_result.ipaddress
                );
            ...
            Err(e) => {
                ...
                println!(
                    "{}Unable to open TCP socket connection to IP Address: '{}' (for server \
                    '{}:{}') - error message: {}",
                    WRN_MSG_PREFIX,
                    ip_check_result.ipaddress,
                    ip_check_result.hostname,
                ...
                if !resume_os_advice_count_given && err_msg.contains("os error 111") {
                    stages_status[stage_index].advice.push(
                        "The type of TCP connection error received indicates that the host \
                        machines are running and generally accessible but the hosted MongoDB \
                        servers are not yet fully up and running and so CANNOT ACCEPT the socket \
                        connection. CHECK THE STATE of the MongoDB deployment servers, in case \
                        there is a problem there. If deployed to Atlas, this situation can happen \
                        if the cluster had been paused and is now resuming or vice versa (if this \
                        is the case, check the Atlas console to see when the cluster is fully \
                        resumed/running, and then just try this connection check again)"
                            .to_string(),
                    );
                ...
                stages_status[stage_index].advice.push(format!(
                    "From this machine launch a terminal and use the netcat tool to see if a \
                    socket can be successfully opened to the server:port:  'nc -zv -w 5 {} {}'",
...
//
async fn stage5_driver_check(
    stage_index: usize, stages_status: &mut [StageStatus], url: &str, usr: Option<&str>,
    pwd: Option<&str>,
    ...
    let client_options = match get_mongo_client_options(url, usr, pwd).await {
        ...
        Err(e) => {
            println!(
                "{}The driver found an issue in the specified URL '{}' when trying to parse and \
                process it - error message: {}",
                ERR_MSG_PREFIX,
                url,
                e.to_string()
            );
...
async fn stage6_dbping_check(
    ...
    match get_dbping_response(client_options).await {
        Ok(dbping_response) => {
            ...
            if dbping_ok {
                println!(
                    "{}The driver successfully connected to the deployment, with the 'dbping' \
                ...
                println!("{}{}", ERR_MSG_PREFIX, MSG);
                stages_status[stage_index].advice.push(
                    "Using the Mongo Shell, connect to the MongoDB deployment and run the \
                    'db.runCommand({ping: 1})' command to check the health of the deployment and \
        ...
        Err(e) => {
            ...
            match e.downcast_ref::<MongoError>() {
                ...
                None => println!(
                    "{}The driver was unable to establish a valid network connection to the \
                    MongoDB deployment - error message: {}",
                    ERR_MSG_PREFIX,
                    e.to_string()
                ),
            }
...
//
async fn stage7_health_check(
    stage_index: usize, stages_status: &mut [StageStatus], client_options: &ClientOptions,
    shared_tier: bool,
) -> Result<(), Box<dyn Error>> {
    print_stage_header(stage_index);
    stages_status[stage_index].state = StageState::Failed;
    ...
    const ADVC: &str = "Using the Mongo Shell, connect to the MongoDB deployment and run the \
                       'db.isMaster()' command to check the health of the deployment and to see if \
    ...
    match get_db_ismaster_response(client_options).await {
        Ok(doc) => {
            if let Ok(msg) = doc.get_str("msg") {
                if msg == "isdbgrid" {
                    println!(
                        "{}Issued command 'ismaster' indicates that a mongos router has been \
                        connected to and the deployment is Sharded",
                        INF_MSG_PREFIX
                    );
                    is_identified = true;
                }
            ...
            if !is_identified && shared_tier {
                println!(
                    "{}MongoDB response indicates that the deployment is an Atlas shared tier (M0, \
                    M2 or M5) based on a shared replica set",
                    INF_MSG_PREFIX
                );
                is_identified = true;
            }
            ...
            if !is_identified {
                if let Ok(primary) = doc.get_str("primary") {
                    if let Ok(hosts) = doc.get_array("hosts") {
                        println!(
                            "{}Issued command 'ismaster' indicates that a replica set deployment \
                        ...
                        for address_bson in hosts {
                            ...
                            println!(
                                "{}Issued command 'ismaster' lists one of the replica set members \
                                as: '{}' ({})",
                                INF_MSG_PREFIX, host, mmbr_type
                            );
                            is_identified = true;
                        }
                    }
            ...
            if !is_identified {
                if let Ok(ismaster) = doc.get_bool("ismaster") {
                    if ismaster {
                        println!(
                            "{}Issued command 'ismaster' indicates that a standalone mongod server \
                            deployment has been connected to ",
                            INF_MSG_PREFIX
                        );
                        is_identified = true;
                    }
        ...
        Err(e) => {
            println!(
                "{}The driver received an error when trying to issue the 'ismaster' command - \
                error message: {}",
                ERR_MSG_PREFIX,
                e.to_string()
            );
...
//
async fn try_open_client_tcp_connection(
    hostname: String, port: u16, ipaddress: IpAddr,
) -> TCPCheckResult {
...
//
async fn get_mongo_client_options(
...
//
async fn get_db_ismaster_response(
    client_options: &ClientOptions,
    ...
    match ping(hostname) {
        PingResult::ConnectionSuccess => {
            if tcp_connected_succesfully {
                println!(
                    "{}ICMP ping successfully achieved to host '{}'",
                    INF_MSG_PREFIX, hostname
                );
            } else {
                println!(
                    "{}Despite a TCP connection to the host failing, an ICMP ping was successfully \
                    achieved to host '{}' which implies that the host server is up and running but \
                    either: 1) TCP connections to a specific port are blocked by a firewall (or \
                    lack of valid Atlas access list entry), or 2) the mongod process is not \
                    running and listening on the specific port",
                    INF_MSG_PREFIX, hostname
                );
            }
        }
        PingResult::ConnectionFailure(message) | PingResult::DNSIssue(message) => {
            if tcp_connected_succesfully {
                println!(
                    "{}ICMP ping to host '{}' not achieved, but because a TCP connection to the \
                    host was successful, this is probably just because ICMP pings are blocked by \
                    the server - error message: {}",
                    INF_MSG_PREFIX, hostname, message
                );
            } else {
                println!(
                    "{}ICMP ping to host '{}' not achieved - error message: {}",
        ...
        PingResult::OSCmndIssue(message) => {
            if tcp_connected_succesfully {
                println!(
                    "{}Unable to run the OS ping executable on this machine to test an ICMP ping \
                    to the host '{}', however, because a TCP connection to the host was successful\
                    , this is not an issue - error message: {}",
                    INF_MSG_PREFIX, hostname, message
                );
            } else {
                println!(
                    "{}Unable to run the OS ping executable on this machine to test an ICMP ping \
                    to the host '{}' - error message: {}",
                    WRN_MSG_PREFIX, hostname, message
                );
            }
        }
...
fn capture_no_connection_advice(stg: &mut StageStatus) {
    stg.advice.push(
        "If using Atlas, via the Atlas console, in the 'Network Access' section, for the 'IP \
        Access List' tab ensure this machine is listed in the access list, and if not, add it ('\
        Add Current IP Address')"
            .to_string(),
    );
    stg.advice.push(
        "If using Atlas, via the Atlas console, check the cluster is NOT PAUSED and resume it if \
        is paused"
            .to_string(),
    );
    stg.advice.push(
        "If not using Atlas to host the MongoDB deployment, check the firewall rules for the \
        network hosting the deployment to ensure it permits MongoDB TCP connections on the 
        configured ports - check NOT BLOCKED"
            .to_string(),
    );
    stg.advice.push(
        "Check any local firewalls on this machine and in your local network, to ensure that \
        MongoDB TCP network connections to outside your network are NOT BLOCKED"
            .to_string(),
    );
}
...
fn alert_on_db_error_type(stg: &mut StageStatus, url: &str, err: &MongoError) {
    ...
    match &*kind.to_owned() {
        MongoErrorKind::ServerSelectionError { message, .. } => {
            if message.contains("os error 104") {
                println!(
                    ...
                    but given that a TCP connection was achieved in an earlier stage, this may \
                    indicate that the deployment is an Atlas shared tier (M0, M2 or M5) based on a \
                    shared replica set, but currently with no matching IP Access List entry \
                    defined to enable access from this machine.   Detail: {}",
                    ERR_MSG_PREFIX, message
                );
            } else if message.contains("InvalidDNSNameError") {
                println!(
                    "{}The driver was unable to establish an appropriate connection to the 
                ...
                println!(
                    "{}The driver was unable to establish a valid connection to the deployment, \
                    but given that a TCP connection was achieved in an earlier stage, this may \
                    indicate that the deployment is configured with TLS/SSL but the URL used does \
                    not reflect this, and/or is an Atlas shared tier (M0, M2 or M5) based on a \
                    shared replica set, but currently with no matching IP Access List entry \
                    defined to enable access from this machine.   Detail: {}",
                    ERR_MSG_PREFIX, message
                );
                ...
                println!(
                    "{}The driver was unable establish a valid connection to any server in the \
                    MongoDB deployment, so cannot perform a server selection.   Detail: {}",
                    ERR_MSG_PREFIX, message
                );
            }
        ...
        MongoErrorKind::AuthenticationError { message, .. } => {
            ...
            stg.advice.push(
                "Check the USERNAME you provided in the MongoDB URL or as a parameter, if you \
                specified one, to ensure it matches a configured database user in the target \
                MongoDB deployment"
                    .to_string(),
            );
            stg.advice.push(
                "Check the PASSWORD you provided in the MongoDB URL or as a parameter, if you \
                specified one, to ensure it is correct for the configured database user in the \
                target MongoDB deployment"
                    .to_string(),
            );
            stg.advice.push(
                "Check an 'authSource' URL option has been provided (e.g. '&authSource=admin') and \
                its value is correct for the configured target MongoDB deployment"
                    .to_string(),
            );
        }
        ...
        _ => {
            ...
            println!(
                "{}The driver was unable to establish a valid connection to the MongoDB deployment\
                .   Detail:: {}",
                ERR_MSG_PREFIX, err_msg
            );
            capture_older_atlas_versions_advice_if_affected(stg, &err_msg);
            stg.advice.push(
                "Check the FIREWALL RULES of both your client network and the network hosting the \
                MongoDB deployment, for rules that prevent MongoDB TCP connections to the MongoDB \
                deployment member ports"
                    .to_string(),
            );
        }
    };
...
fn capture_some_optional_advice_if_affected(stg: &mut StageStatus, url: &str, errmsg: &str) {
    if errmsg.contains("os error 104") {
        stg.advice.push(
            "If the MongoDB deployment is an Atlas M0/M2/M5 tier cluster then via the Atlas console\
            , in the 'Network Access' section, for the 'IP Access List' tab select to 'ADD CURRENT \
            IP ADDRESS' which should be the address of this host machine"
                .to_string(),
        );
    } else if errmsg.contains("InvalidDNSNameError") {
        stg.advice.push(
            "If the MongoDB deployment is an Atlas M0/M2/M5 tier cluster then ensure in the URL \
            you are defining a 'mongodb+srv://' SRV address or a list of DNS hostnames and NOT a \
            list of IP addresses, because shared tiers cannot determine which tenant you want to \
            reference from just a list of IP Addresses in a URL"
                .to_string(),
        );
    ...
    {
        stg.advice.push(
            "The type of error received indicates that the MongoDB deployment may be configured \
            with TLS (aka SSL) which it should be, for security reasons. However, the MongoDB URL \
            you provided does not seem to indicate that the driver should communicate via TLS. ADD \
            THE CLIENT OPTION 'tls=true' to the MongoDB URL you specify, as per the following \
            format specification, and try again: \
            https://docs.mongodb.com/manual/reference/connection-string/"
                .to_string(),
        );
    }
    ...
    if !url.contains('?') {
        stg.advice.push(
            "Ensure you have used a ? (QUESTION MARK) in the MongoDB URL you specified, before any \
            connection string options you defined, and also check each of these options is VALID \
            as per the format specification documented at: 
            https://docs.mongodb.com/manual/reference/connection-string/"
                .to_string(),
        );
    }
...
fn capture_older_atlas_versions_advice_if_affected(stg: &mut StageStatus, errmsg: &str) {
    if errmsg.contains("tls handshake eof") {
        stg.advice.push(
            "UNSUPPORTED by this utility - it appears that you are targetting a MongoDB cluster \
            deployed in Atlas, which is running a version of MongoDB lower than 4.2 (e.g. version \
            3.6 or 4.0). Unfortunately, due to older TLS/SSL libraries used on the hosts' OS for \
            those MongoDB versions, in Atlas, the Rust driver, used by this connection test utility\
...
fn print_slow_dns_warning_if_on_windows() {
    if cfg!(windows) {
        println!(
            "{}A slow DNS lookup might now occur (a behaviour on Windows OS only), please be 
...
mod tests {
    ...
    fn unit_test_extract_cluster_seedlist() {
        ...
        assert_extract_cluster_seedlist("mongodb//abc123.mongodb.net:27017/test", &[], &[]);
        assert_extract_cluster_seedlist(
            "mongodb://abc123.mongodb.net:27017/test",
            &["abc123.mongodb.net"],
            &[27017],
        );
        assert_extract_cluster_seedlist(
            "mongodb://abc123.mongodb.net:27017/",
            &["abc123.mongodb.net"],
            &[27017],
        );
        assert_extract_cluster_seedlist(
        ...
        assert_extract_cluster_seedlist(
            "mongodb://abc123.mongodb.net/test",
            &["abc123.mongodb.net"],
            &[],
        );
        assert_extract_cluster_seedlist(
            "mongodb://abc123.mongodb.net/",
            &["abc123.mongodb.net"],
            &[],
        );
        assert_extract_cluster_seedlist(
        ...
        assert_extract_cluster_seedlist(
            "mongodb+srv://seb360.o5qhl.gcp.mongodb.net:27017/test",
            &["seb360.o5qhl.gcp.mongodb.net"],
            &[27017],
        );
        assert_extract_cluster_seedlist(
            "mongodb+srv://seb360.o5qhl.gcp.mongodb.net:27017/",
            &["seb360.o5qhl.gcp.mongodb.net"],
            &[27017],
        );
        assert_extract_cluster_seedlist(
        ...
        assert_extract_cluster_seedlist(
            "mongodb+srv://seb360.o5qhl.gcp.mongodb.net/test",
            &["seb360.o5qhl.gcp.mongodb.net"],
            &[],
        );
        assert_extract_cluster_seedlist(
            "mongodb+srv://seb360.o5qhl.gcp.mongodb.net/",
            &["seb360.o5qhl.gcp.mongodb.net"],
            &[],
        );
        assert_extract_cluster_seedlist(
        ...
        assert_extract_cluster_seedlist(
            ...
            authSource=authDB",
            &["mongodb1.example.com", "mongodb2.example.com"],
            &[27317, 27017],
        );
        assert_extract_cluster_seedlist(
            "mongodb://mongodb1.example.com:27317,mongodb2.example.com\
            :27017/",
            &["mongodb1.example.com", "mongodb2.example.com"],
            &[27317, 27017],
        );
        ...
        assert_extract_cluster_seedlist(
            "mongodb://mongodb0.example.com:27017,mongodb1.example.com:27017,mongodb2.example.com:\
            27017/?replicaSet=myRepl&authSource=admin",
        ...
        assert_extract_cluster_seedlist(
            "mongodb://myuser@mongodb0.example.com:27017,mongodb1.example.com:27017,\
            mongodb2.example.com:27017/?replicaSet=myRepl&authSource=admin",
            &["mongodb0.example.com", "mongodb1.example.com", "mongodb2.example.com"],
            &[27017, 27017, 27017],
        );
        assert_extract_cluster_seedlist(
            "mongodb://myuser:mypassword@mongodb0.example.com:27017,mongodb1.example.com:27017,\
            mongodb2.example.com:27017/?replicaSet=myRepl&authSource=admin",
            &["mongodb0.example.com", "mongodb1.example.com", "mongodb2.example.com"],
            &[27017, 27017, 27017],
        );
        assert_extract_cluster_seedlist(
            "mongodb://myuser:mypassword@mongodb0.example.com,mongodb1.example.com,\
            mongodb2.example.com/?replicaSet=myRepl&authSource=admin",
            &["mongodb0.example.com", "mongodb1.example.com", "mongodb2.example.com"],
            &[],
        );
        assert_extract_cluster_seedlist(
            "mongodb+srv://main_user:Password1@/test?retryWrites=true&w=majority",
            &[],
            &[],
        );
    }
    ...
    fn integration_test_real_atlas_shared_tier_srv() {
        ...
        let url = format!(
            "mongodb+srv://main_user:{}@sandboxcluster.s703u.mongodb.net/test",
    ...
    fn integration_test_real_atlas_shared_tier_list() {
        // Expects Atlas M0/M2/M5 shared tier called 'sandboxcluster'
        let url = format!(
            "mongodb://main_user:{}@sandboxcluster-shard-00-02.s703u.mongodb.net:27017,\
            sandboxcluster-shard-00-01.s703u.mongodb.net:27017,sandboxcluster-shard-00-00.s703u.\
            mongodb.net:27017/test?tls=true&authSource=admin",
            get_test_password_panicking_if_missing()
        );
        start(&url, None, None);
    }
    ...
    fn integration_test_real_atlas_shared_tier_ipaddresses_list() {
        // Expects Atlas M0/M2/M5 shared tier called 'sandboxcluster'
        let url = format!(
            "mongodb://main_user:{}@63.34.95.190:27017,34.252.0.188:27017,63.35.156.164:27017/test?\
    ...
    fn integration_test_real_atlas_repset_srv() {
        ...
        let url = format!(
            "mongodb+srv://main_user:{}@testcluster.s703u.mongodb.net/test?retryWrites=true&\
            w=majority",
            get_test_password_panicking_if_missing()
        );
        start(&url, None, None);
    ...
    fn integration_test_real_atlas_repset_only_one_valid_host() {
        // Expects Atlas M10+ replica set called 'testcluster'
        let url = format!(
            "mongodb://main_user:{}@testcluster-BAD-shard-00-00.s703u.mongodb.net:27017,testcluster\
            -BAD-shard-00-01.s703u.mongodb.net:27017,testcluster-shard-00-02.s703u.mongodb.net:\
            27017/?tls=true",
            get_test_password_panicking_if_missing()
        );
        start(&url, None, None);
    ...
    fn integration_test_real_atlas_sharded_srv() {
        ...
        let url = format!(
            "mongodb+srv://main_user:{}@testcluster2.s703u.mongodb.net/test?retryWrites=true&w=\
            majority",
            get_test_password_panicking_if_missing()
        );
        start(&url, None, None);
    ...
    fn integration_test_real_atlas_sharded_only_one_valid_host() {
        // Expects Atlas M30+ sharded cluster called 'testcluster2'
        let url = format!(
            "mongodb://main_user:{}@testcluster2-shard-01-01.s703u.mongodb.net:27016/?tls=true",
            get_test_password_panicking_if_missing()
        );
        start(&url, None, None);
    }
    ...
    fn integration_test_real_atlas_sharded_missing_tls() {
        // Expects Atlas M30+ sharded cluster called 'testcluster2'
        let url = format!(
            "mongodb://main_user:{}@testcluster2-shard-01-01.s703u.mongodb.net:27016/",
            get_test_password_panicking_if_missing()
        );
        start(&url, None, None);
    }
    ...
    fn integration_test_real_atlas_cluster_bad_passwd() {
        // Expects Atlas M10+ replica set called 'testcluster'
        start("mongodb+srv://main_user:badpswd@sandboxcluster.s703u.mongodb.net/test", None, None);
    }
    ...
    fn get_test_password_panicking_if_missing() -> String {
        let err_msg = format!(
            "Error retrieving value from OS environment variable '{}' which is intended for use as \
            the password in a test, to connect to a MongoDB cluster - ensure this environment \
